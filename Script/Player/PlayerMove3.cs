...
public class PlayerMove3 : MonoBehaviour {
   ...
   // public AudioSource source;//音效
    ...
    void Move()
    {
        if (Input.GetKeyUp(KeyCode.D)&&(this.transform.position.z<57||this.transform.position.x>7))
        {
            if (!isMove&&!die)
            {
                if (speedUp&&this.transform.position.z<54|| this.transform.position.x > 7)
                {
